class JourneysController < ApplicationController
  ...
  def completed
    @disorder = Disorder.find(params[:id]) # hard-coded for now, to be able to access journeys/completed directly; instead of the number it should be: params[:id]
    @enemies = Enemy.where(disorder_id: params[:id]) # hard-coded for now, to be able to access journeys/completed directly; instead of the number it should be: params[:id]
    @enemy = @enemies.last
  end
